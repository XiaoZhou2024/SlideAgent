...
class ConclusionGenerator:
    ...
    def _create_conclusion_prompt_template(self) -> ChatPromptTemplate:
        ...
        return ChatPromptTemplate.from_messages([
            ("system", """
            你需要参考给定的模板数据和模板结论，给新的数据生成结论，具体要求如下：
            1. 数据理解: 首先，请仔细理解提供的模板数据结构和对应的模板结论，识别其中的关键变量（如地点、数字等）和结论生成逻辑。
            2. 信息替换: 然后，针对新提供的数据，准确替换模板结论中的地点、数字等关键信息，特别注意"一般大于140㎡以上称为改善户型"这一专业定义。
            3. 结论生成: 最后，确保生成的新结论在保持原有逻辑结构的同时，与新数据完全一致，并以清晰、专业的语言呈现。
